...
gulp.task('optimize:styles', () => {
  return gulp
    .src(config.optimize.styles.src)
...
gulp.task('optimize:scripts', () => {
  return gulp
    .src(config.optimize.scripts.src)
    .pipe(plumber({ errorHandler: errorHandler }))
...
gulp.task('optimize', ['optimize:styles', 'optimize:scripts', 'optimize:html']);
